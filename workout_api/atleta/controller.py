...
from workout_api.atleta.schemas import AtletaIn, AtletaOut, AtletaUpdate, AtletaAllOut
...
async def query(
    db_session: DatabaseDependency,
) -> LimitOffsetPage[AtletaAllOut]:
    atletas: list[AtletaAllOut] = (
...
async def query_by_nome(
    nome: Annotated[str, Query(min_length=3)],
    db_session: DatabaseDependency,
) -> list[AtletaOut]:
    atletas = (
        (
            await db_session.execute(
                select(AtletaModel).where(AtletaModel.nome.like(f"%{nome}%"))
            )
        )
        .scalars()
        .all()
...
async def query_by_cpf(
    cpf: Annotated[str, Query(min_length=11)],
    db_session: DatabaseDependency,
) -> AtletaOut:
    atleta: AtletaOut = (
    ...
    if not atleta:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Atleta não encontrada no cpf: {cpf}",
        )
